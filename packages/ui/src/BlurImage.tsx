import Image from 'next/image';
import { ImageProps } from 'next/image';

const defaultBlur =
  'data:image/png;base64,';

type BlurImageProps = ImageProps & Omit<ImageProps, 'placeholder' | 'blurDataURL'>;

export default function BlurImage({ blurDataURL, ...props }: BlurImageProps) {
  return <Image {...props} placeholder="blur" blurDataURL={blurDataURL || defaultBlur} />;
}
